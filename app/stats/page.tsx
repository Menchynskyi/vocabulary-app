...
export default async function Stats({ searchParams }: StatsProps) {
  ...
  const [cardsDailyWords, matchUpStats, blanksStats, availableYears] =
    await Promise.all([
  ...
  if (safeGamesYear !== gamesYear) {
    redirect(
      `/stats?cardsPage=${cardsPage}&matchUpPage=${matchUpPage}&blanksPage=${blanksPage}&gamesYear=${safeGamesYear}`,
    );
  }
  ...
  if (
    ...
    redirect(
      `/stats?cardsPage=${cardsDailyWords.totalPages}&matchUpPage=${matchUpPage}&blanksPage=${blanksPage}&gamesYear=${safeGamesYear}`,
    );
  }

  if (matchUpPage > matchUpStats.totalPages && matchUpStats.totalPages !== 0) {
    redirect(
      `/stats?cardsPage=${cardsPage}&matchUpPage=${matchUpStats.totalPages}&blanksPage=${blanksPage}&gamesYear=${safeGamesYear}`,
    );
  }

  if (blanksPage > blanksStats.totalPages && blanksStats.totalPages !== 0) {
    redirect(
      `/stats?cardsPage=${cardsPage}&matchUpPage=${matchUpPage}&blanksPage=${blanksStats.totalPages}&gamesYear=${safeGamesYear}`,
    );
  }
    ...
    <div className="mx-auto flex w-[90vw] max-w-[1200px] flex-col gap-4 pb-6 pt-0">
      ...
      <div className="rounded-md border">
        <div className="flex flex-col space-y-1.5 p-6">
          ...
          <p className="text-sm text-muted-foreground">
            Record counts for cards, match-up, and blanks in the current year
          </p>
        </div>
        <Suspense fallback={<StatsSkeleton />}>
